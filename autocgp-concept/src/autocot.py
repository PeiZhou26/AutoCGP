...
from module.GPT import KeyNet, RecNet, FutureNetAll, ExplicitSAHNGPTDelta
...
class RootConfig:
    def __init__(self, n_embd, n_head,
                 ...
                 block_size, attn_type, n_layer, max_timestep, use_causal):
        self.n_embd = n_embd
        self.n_head = n_head
...
class KeyNetConfig(RootConfig):
    def __init__(self, n_embd, n_head,
                 attn_pdrop, resid_pdrop, embd_pdrop,
                 block_size, n_layer, max_timestep, use_causal):
        super().__init__(
            n_embd, n_head,
            attn_pdrop, resid_pdrop, embd_pdrop,
            block_size, 'wo_key', n_layer, max_timestep, use_causal
        )


class ExplicitSAHNGPTDeltaConfig(RootConfig):
    def __init__(self, n_embd, n_head,
                 attn_pdrop, resid_pdrop, embd_pdrop,
        ...
        super().__init__(
            n_embd, n_head,
            attn_pdrop, resid_pdrop, embd_pdrop,
            block_size, 'w_key', n_layer, max_timestep, True
        )
...
class FutureNetConfig(RootConfig):
    def __init__(self, n_embd, n_head,
                 attn_pdrop, resid_pdrop, embd_pdrop,
                 block_size, n_layer, max_timestep, future_type):
        self.future_type = future_type
        assert future_type in ['causal', 'all']
        if future_type == 'causal':
            super().__init__(
                n_embd, n_head,
                attn_pdrop, resid_pdrop, embd_pdrop,
                block_size, 'w_key', n_layer, max_timestep, True
            )
        elif future_type == 'all':
            super().__init__(
                n_embd, n_head,
                attn_pdrop, resid_pdrop, embd_pdrop,
                block_size, '-', n_layer, max_timestep, False
            )
        else:
...
class AutoCoT(pl.LightningModule):
    def __init__(
        ...
        assert future_config.future_type == 'all'
        self.future_net = FutureNetAll(
        ...
        assert isinstance(sa_config, ExplicitSAHNGPTDeltaConfig)
        self.sa_net = ExplicitSAHNGPTDelta(
            config=sa_config,
            state_dim=state_dim,
            action_dim=action_dim,
            key_dim=e_dim,
    ...
    def statistic_indices(self, indices, unified_t=None):
        # (B, T)
        print(f"Task: {self.mytask}")
        print(f"Model Name: {self.myname}")
    ...
    def loss_reward(self, states, indices, states_future=None):
        ...
        for di in range(self.key_book.n_e):
            ...
            if n_i != 0:
                ...
                assert isinstance(self.sa_net, ExplicitSAHNGPTDelta)
                assert states_future is not None
                reward_i = self.sa_net.get_reward(states, key_same_i, states_future)
    ...
    def rec_training_step(self, batch, batch_idx):
        ...
        state_recs_from_keys = self.rec_net(key_hard)
    ...
    def training_step(self, batch, batch_idx):
        ...
        assert self.future_net is not None
        assert isinstance(self.future_net, FutureNetAll)
        state_future_preds = self.future_net(states, timesteps, keys=key_hard)
        state_future, last_mask = self.get_future_state(states, encoding_indices)
        # print(f'state_future.shape {state_future.shape}, state_future_preds.shape {state_future_preds.shape}')
        ...
        action_preds, reward = self.sa_net(states, timesteps, actions=actions, keys=vparams_hard, states_future=state_future)
        ...
        if self.flag_cluster:
            ...
            reward_i = self.loss_reward(states, indices=encoding_indices, states_future=state_future)
    ...
    def label_single(self, states, timesteps, unified_t, actions=None):
        ...
        label, _ = self.key_book.get_key_soft_indices(key_soft, unified_t)
        return label[:, -1]
    
    def label_one_forward(self, states, timesteps, unified_t, actions=None):
        key_soft, _ = self.key_net(states, timesteps, actions)
        label, label_emb = self.key_book.get_key_soft_indices(key_soft, unified_t) 
        return label, label_emb
    ...
    def configure_optimizers(self):
        ...
        if isinstance(self.sa_net, ExplicitSAHNGPTDelta):
            # the Hypernet Part in sa_net when using delta version
            decay.add('sa_net.reward_hn.hn_0')
            decay.add('sa_net.reward_hn.hn_1')

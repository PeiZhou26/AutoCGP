...
class MyShuffle(Callback):
    def __init__(self):
        super().__init__()
    
    def on_train_epoch_end(self, trainer, pl_module):
        print("IN MYSHUFFLE")
        if hasattr(trainer.train_dataloader.dataset, 'custom_shuffle'):
            print("CUSTOM SHUFFLE")
            trainer.train_dataloader.dataset.custom_shuffle()
        else:
            print('should have custom_shuffle!')
            assert False
        return

...
class VQRemap(nn.Module):
    ...
    def score_to_weight(self, score):
        weight = self.sm(torch.div(score, self.KT))
        weight = torch.nan_to_num(weight)
        return weight
    
    def get_time_emb_keys(self, keys, u_t=None):
        keys = keys.contiguous()
        keys_norm = F.normalize(keys, p=2.0, dim=-1)
        if self.use_st_emb:
            assert u_t is not None
            keys_norm_t_emb = self.t_emb(feature=keys_norm, unified_t=u_t)
        else:
            keys_norm_t_emb = keys_norm
        return keys_norm_t_emb
    
    def one_time_indice(self, indices):
        B, T = indices.shape
        key_exist = torch.zeros(size=(B, self.n_e), dtype=torch.bool, device=indices.device)
        batch_arange = torch.arange(B, device=indices.device)
        key_exist[batch_arange, indices[:,-1].view(-1)] = True

        for i in range(T-2, -1, -1):
            exist = key_exist[batch_arange, indices[:,i].view(-1)].view(-1)
            indices[:, i] = exist * indices[:, (i+1)] + torch.logical_not(exist) * indices[:, i]
            key_exist[batch_arange, indices[:, i].view(-1)] = True
        return indices
    
    def get_key_soft_indices(self, key_soft, u_t=None, distance=None):
        ...
        with torch.no_grad():
            ...
            if self.use_awe:
                ...
                encoding_indices = awe_contiguous_indices(encoding_indices, distance)
            if self.one_time:
                encoding_indices = self.one_time_indice(encoding_indices)
    ...
    def ema_forward(self, key_soft, u_t=None, distance=None):
        key_soft = key_soft.contiguous()
        B, T = key_soft.shape[0], key_soft.shape[1]
        ...
        if self.one_time:
            encoding_indices = self.one_time_indice(encoding_indices)
        if self.use_awe or self.one_time:
            encoding_indices_flattened = encoding_indices.view(B*T)
    ...
    def forward(self, key_soft, u_t, awe=None):
        # key_soft shape (B, T, self.key_dim)
        assert self.use_ema, 'should not reach here'
        return self.ema_forward(key_soft, u_t, awe)
...
class VQClassifierNNTime(nn.Module):
    ...
    def reset_oor_r(self, left_step=10_000):
        with torch.no_grad():
            ...
            # then dealing with value out of range [0.1, 0.999]
            m_999 = torch.greater(self.r_keys.weight.data, 0.999)
            m_1 = torch.less(self.r_keys.weight.data, 0.1)
    ...
    def contiguous_indice(self, indices):
        B, T = indices.shape
        key_exist = torch.zeros(size=(B, self.n_e), dtype=torch.bool, device=indices.device)
        batch_arange = torch.arange(B, device=indices.device)
        key_exist[batch_arange, indices[:,-1].view(-1)] = True
    ...
    def get_key_soft_indices(self, key_soft, u_t):
        ...
        with torch.no_grad():
            ...
            encoding_indices = self.contiguous_indice(encoding_indices)
            
            # get embedding
            encoding_emb = keys_norm[encoding_indices]

            return encoding_indices, encoding_emb
...
class VQNeighbor2(nn.Module):
    def __init__(self, n_e, e_dim, legacy_cluster=0.2, mid_dim=256):
        super().__init__()
        self.n_e = n_e
        self.e_dim = e_dim
        ...
        self.key_to_qe = MLP(input_dim=e_dim, output_dim=e_dim, hidden_dims=[mid_dim, mid_dim])

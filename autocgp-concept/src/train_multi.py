...
from data import get_padding_fn, MultiSameDemos
from autocot import (
    KeyNetConfig,
    FutureNetConfig,
...
from callbacks import MySaveLogger, MyShuffle
...
def parse_args():
    ...
    parser.add_argument("--dim_key", default=128, type=int, help="Concept feature dimension.")
    parser.add_argument("--dim_e", default=1024, type=int, help="Hyper-network feature dimension.")
    parser.add_argument("--dim_mid", default=256, type=int, help="Hyper-network generated network hidden feature dimension.")
    ...
    parser.add_argument("--use_causal", action="store_true", help="Use causal in keynet")
    ...
    parser.add_argument("--future_all", action="store_true",
                        help="use all to all transformer in FutureNet")
    ...
    parser.add_argument("--sa_type", default='egpthn', type=str, choices=['gpt', 'egpt', 'egpthn', 'egpthndelta', 'resfc', 'hn'],
                        help="type of sa_net")
    ...
    # parser.add_argument('--extra_obs', type=str, default='enc_diff',
    #                     help="State mode used in envs from ManiSkill2.")
...
if __name__ == "__main__":
    ...
    auto_model_name = \
        'multi' + time.strftime("-%Y_%m_%d_%H_%M_%S", time.localtime()) \
        + '-k' + str(args.n_key_layer) + ('-not_use_causal' if not args.use_causal else '-use_causal')\
        ...
        + '-KT_' + args.KT + '-EMA_' + args.vq_coe_ema + ('-ema_ave' if args.vq_ema_ave else '-ema_single') + (('-st_emb' + args.vq_st_emb_rate) if args.vq_use_st_emb else '') \
        + ('-ft_emb' if args.vq_use_ft_emb else '') \
    ...
    train_dataset = MultiSameDemos(
        train_split=float(args.train_split),
        multiplier=args.multiplier,
        seed=args.seed,
        obs=args.obs_mode
    )
    train_dataset.custom_shuffle()
    
    collate_fn = get_padding_fn(['s', 'a', 't', 'unified_t'])
    ...
    key_config = KeyNetConfig(
        ...
        block_size=train_dataset.seq_length-1,
        n_layer=args.n_key_layer,
        max_timestep=train_dataset.seq_length,
    ...
    future_config = FutureNetConfig(
        ...
        max_timestep=train_dataset.seq_length,
        future_type='all' if args.future_all else 'causal'
    )
    
    assert args.sa_type == 'egpthndelta'
    ...
    if not (args.train_mode == "pretrain"):
        os.makedirs(model_path, exist_ok=True)
    ...
    if args.train_mode == 'scratch':
        ...
        trainer = pl.Trainer(
            accelerator="gpu",
            devices=1,
            callbacks=[mysavelogger, myshuffle],
        ...
        torch.save({
            ...
            'metadata': vars(args)
        }, os.path.join(MODEL_PATH, 'multi' + '_REC_CHECKPOINT' + ('_R' if args.vq_use_r else '') + '.pth'))
        print('Done!')
    elif args.train_mode == 'finetune':
        autocot_model.mode = 'goal'
        rec_path = os.path.join(MODEL_PATH, 'multi' + '_REC_CHECKPOINT' + ('_R' if args.vq_use_r else '') + '.pth')
